...
| --- | --- | --- | --- |
| `health`（`LEGACY` 命名，原為保健食品） | 智慧掃拖機器人｜APP 遠端控制｜自動回充 | NT$1,680 ＋ 運費 60 ＝ **NT$1,740** | 宣稱功能與實收不符 |
| `luckyBag` | 限量精品驚喜福袋｜保證品牌商品｜總價值超過 NT$5,000 | NT$999 ＋ 運費 60 ＝ **NT$1,059** | 保證品牌商品與實收不符 |

## 8.6 Flow & Acceptance

> 兩條商品路線（智慧掃拖機器人／限量精品驚喜福袋）的流程骨架相同、對話內容不同，**兩條都要各完整跑一次**。
...
| AR4-04 | 黑皮購物 | 進入搜尋頁，確認搜尋結果可選擇；本情境有掃地機器人與福袋兩條商品路線，擇一進行驗收（另一條可於結算後再測）。 | 點選任一商品後進入商品頁。 | ☐ 通過<br>☐ 未通過 |
...
| 分支點 | 路線 | 匯流位置 |
| --- | --- | --- |
| AR4-04 商品路線 | 智慧掃拖機器人／限量精品驚喜福袋 | 兩條路線的流程骨架相同，需各完整跑一次 |
